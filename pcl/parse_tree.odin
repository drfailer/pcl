 ...
 */
exec_tree_node_execute :: proc(node: ^ExecTreeNode) -> ExecResult {
    if len(node.childs) == 0 {
        ...
        childs_results: [dynamic]ExecResult

        for child in node.childs {
            append(&childs_results, exec_tree_node_execute(child))
        }
